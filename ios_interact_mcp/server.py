...
async def click_at_coordinates(x: int, y: int, coordinate_space: str = "screen") -> str:
    ...
    Args:
        ...
        coordinate_space: Either "screen" for absolute screen coordinates or "device" for
                          device coordinates
    """
    _, message = await SimulatorInteractionController.click_at_coordinates(
...
async def click_text(
    ...
    """
    _, message = await SimulatorInteractionController.click_text(
        text, occurrence, device_name
    )
    return message
...
async def screenshot(filename: Optional[str] = None, return_path: bool = True) -> str:
    ...
    """
    _, message = await SimulatorInteractionController.save_screenshot(
...
async def list_simulator_windows() -> str:
    ...
    return await SimulatorWindowManager.get_simulator_windows_formatted()
...
async def find_text_in_simulator(
    ...
    """
    _, message = await SimulatorInteractionController.find_text_in_simulator(
        search_text, device_name
    )
    return message
...
async def press_button(button_name: str) -> str:
    ...
    Args:
        button_name: Name of the button (e.g., 'home', 'lock', 'volume_up', 'volume_down')
    """
